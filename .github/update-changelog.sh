...
repo=${string#$prefix}
repo=${repo%$suffix}
url=$(curl -v -d '{"title":"Changelog Update","head":"changelog_update","base":"master"}' -H "Content-Type: application/json" -H "Authorization: token ${CHANGELOG_GITHUB_TOKEN}" https://api.github.com/repos/${repo}/pulls | jq '.url')
echo $url
if [ -z ${url+x} ]; then
